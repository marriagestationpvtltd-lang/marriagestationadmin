...
// Check if photos should be blurred
  bool get shouldBlurPhotos {
    // Case 1: Free user - always blur
    if (!isCurrentUserPaid) return true;

    // Case 2: Paid user with not_sent - blur
    if (isPhotoRequestNotSent) return true;

    // Case 3: Paid user with pending - blur
    if (isPhotoRequestPending) return true;

    // Case 4: Paid user with rejected - blur
    if (isPhotoRequestRejected) return true;

    // Case 5: Paid user with accepted - don't blur
    if (isPhotoRequestAccepted) return false;
...
// Check if user can view photos (for UI logic)
  bool get canViewPhotos => isCurrentUserPaid && isPhotoRequestAccepted;
